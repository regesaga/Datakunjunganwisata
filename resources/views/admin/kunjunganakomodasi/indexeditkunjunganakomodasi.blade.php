                ...
                <thead>
                    <tr><th colspan="{{ 3 + (count($kelompok) * 2) + (count($wismannegara) * 2) }}">
                        ...
                        <h2 style="text-align: center; text-transform: uppercase;">
                            Rekap Data Kunjungan {{$akomodasi->first()->namaakomodasi}}
                ...
                <tbody>
                    @foreach ($kunjungan as $tanggal => $dataTanggal)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-info btn-sm" href="{{ route('admin.kunjunganakomodasi.edit', ['akomodasi_id' => $hash->encode($akomodasi->first()->id),'tanggal_kunjungan' => $tanggal]) }}">
                                    <i class="fas fa-pencil-alt"></i> Ubah
                                </a>
                                <a href="{{ route('admin.kunjunganakomodasi.delete', ['akomodasi_id' => $hash->encode($akomodasi->first()->id), 'tanggal_kunjungan' => $tanggal]) }}"
                                 ...
                                 <form id="delete-form" action="{{ route('admin.kunjunganakomodasi.delete', ['akomodasi_id' => $hash->encode($akomodasi->first()->id), 'tanggal_kunjungan' => $tanggal]) }}" method="POST" style="display:none;">
                                     @csrf
                                     @method('DELETE')
                                 </form>
