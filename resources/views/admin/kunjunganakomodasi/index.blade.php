...
<section class="content-header">
    <div class="container-fluid">
        ...
        <form method="GET" action="{{ route('admin.kunjunganakomodasi.index') }}">
            <div class="row">
                <div class="col-lg-3">
                    <label style="text-align: center; text-transform: uppercase;" for="category" class="form-label">Kategori</label>
                    <select name="categoryakomodasi_id" class="form-control select2">
                        <option style="text-align: center; text-transform: uppercase;" value="">SEMUA TEMPAT AKOMODASI</option>
                        @foreach($kategoriAkomodasi as $kategori)
                            <option style="text-align: center; text-transform: uppercase;" value="{{ $kategori->id }}" {{ request('categoryakomodasi_id') == $kategori->id ? 'selected' : '' }}>{{ $kategori->category_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-lg-3">
                ...
                <table id="example1" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <h2 style="text-align: center; text-transform: uppercase;">{{ $kategoriAkomodasi->find($categoryakomodasi_id)->category_name ?? 'Semua Tempat Akomodasi' }} </h2>
                            </th>
                        </tr>
                     
                        <tr>
                            <th rowspan="3" style="text-align: left; text-transform: uppercase;">Akomodasi</th>
                    ...
                    @endphp
                        <tr class="{{  $isZero ? 'bg-navy color-palette' : '' }}">
                                 ...
                                 <td style="text-align: center; text-transform: uppercase;">  <a  href="{{ route('admin.kunjunganakomodasi.indexeditkunjunganakomodasi', [
                                    'akomodasi_id' => $hash->encode($dataTanggal['akomodasi']->id), 
                                    'bulan' => $bulan,
                                    'tahun' => $tahun
                                ]) }}" style="text-align: center; text-transform: uppercase;">{{ $dataTanggal['akomodasi']->namaakomodasi }}
                                ...
                                @endforeach
                                <!-- Data Total Akomodasi Mancanegara -->
