...
<section class="content-header">
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.kunjunganevent.storewisnuevent') }}" method="POST">
            @csrf
        <div class="form-header">
                <h3  style="text-align: center; text-transform: uppercase;">INPUT DATA KUNJUNGAN EVENT</h3>
                <div class="col-lg-2">
                    <select name="event_calendar_id" id="event_calendar_id" class="form-control select2">
                        @if($event->isEmpty())
                            <option value="" disabled selected>Anda belum memiliki Event</option>
                        @else
                            @foreach($event as $item)
                                <option value="{{ $item->id }}" 
                                        data-tanggal="{{ $item->tanggalmulai ? \Carbon\Carbon::parse($item->tanggalmulai)->format('Y-m-d') : '' }}"
                                        {{ request('event_calendar_id') == $item->id ? 'selected' : '' }}>
                                    {{ $item->title }}
                                </option>
                            @endforeach
                        @endif
...
@section('scripts')
    ...
    const eventDropdown = document.getElementById('event_calendar_id');
    const tanggalInput = document.getElementById('tanggal_kunjungan');

    // Set tanggal langsung saat halaman dimuat jika opsi sudah dipilih
    const selectedOption = eventDropdown.options[eventDropdown.selectedIndex];
    const tanggal = selectedOption ? selectedOption.getAttribute('data-tanggal') : '';
    if (tanggal) {
