    ...
    <main id="main">
        ...
        <section id="destinasi" class="destinasi">
            <div class="container">
                ...
                <div class="card card-primary">
                    <div class="card-header">
                      <h3 class="card-title">Pesan Tiket {{$wisata->namawisata}}</h3>
                    </div>
                                ...
                                <div class="bd-example-snippet bd-code-snippet mb-5">
                                  <div class="bd-example m-0 border-0">
                                    ...
                                    <div class="row overflow-auto px-2" id="tickets" style="max-height:280px ">
                                        @foreach ($hargatiket as $index => $hargatiket)
                                                    
                                        <div class="callout callout-primary p-3 my-2 ">
                                                      <div class="row" style="padding-top: 0px">
                                                          <div class="col-6">
                                                            <p class="tix-cat">{{ $hargatiket->kategori }}</p>
                                                            <input type="hidden" name="harga_tiket_id[]"class="form-control" required value="{{ $hargatiket->id }}" readonly>
                                                            <input type="hidden" class="form-control" name="kategori[]" required value="{{ $hargatiket->kategori }}" readonly>
                                                            <input type="hidden" name="harga[]" class="form-control" required value="{{ $hargatiket->harga }}" readonly>
                                                            <strong style="font-size: 14px;" class="text-dark">Rp. {{ number_format($hargatiket->harga, 0, ".", ".") }},- </strong>
                                                          </div>
                                                          <div class="col-6">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3">
                        <input type="hidden" name="wisata_id"class="form-control" required value="{{ $wisata->id }}" readonly>
                        <input type="hidden" name="namawisata"class="form-control" required value="{{ $wisata->namawisata }}" readonly>
...
crossorigin="anonymous"></script>
  ...
  function submitForm() {
    ...
    var formData = {
        _token: '{{ csrf_token() }}',
        wisata_id: $('input[name="wisata_id"]').val(),
        namawisata: $('input[name="namawisata"]').val(),
        ...
        harga_tiket_id: $('input[name="harga_tiket_id[]"]').map(function(){ return this.value; }).get(),
        kategori: $('input[name="kategori[]"]').map(function(){ return this.value; }).get(),
        harga: $('input[name="harga[]"]').map(function(){ return this.value; }).get(),
        jumlah: $('input[name="jumlah[]"]').map(function(){ return this.value; }).get()
    };

    $.post("{{ route('pesantiket.store') }}", formData)
        .done(function(data) {
                ...
                if (formData.metodepembayaran === 'Online') {
                    snap.pay(data.snap_token, {
                        onSuccess: function(result) {
                            ...
                            window.location.href = "{{ route('website.pesantiket.checkout_finish', ['pesantiket' => ':kodetiket']) }}".replace(':kodetiket', data.kodetiket);
                        },
