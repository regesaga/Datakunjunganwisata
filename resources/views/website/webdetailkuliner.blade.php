        ...
        <section id="breadcrumbs" class="breadcrumbs">
            <div class="container">
                ...
                <ol>
                    ...
                    <li>Kuliner Details</li>
                </ol>

            </div>
        </section><!-- End Breadcrumbs -->

        <!-- ======= Kuliner Details Section ======= -->
            ...
            <div class="container">
                <div class="row gy-4">
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="row">
                            @foreach ($kulinerproduk as $item)
                                <div class="col-md-4 mt-3">
                                    <div class="card mb-4 shadow-sm">
                                        <img src="{{ $item->thumbnail }}" class="card-img-top" alt="testing" height="220px">
                                        <div class="card-body">
                    ...
                    <div class="col-lg-5">
                        <span class="fas fa-eye mr-1"></span>
                            {{ __('messages.webView') }} {{ $kuliner->views }}
                            
                            
                            <div class="card card-primary">
                                <div class="card-header">
                                  <h3 class="card-title">Kuliner information</h3>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                  <strong><i class="fas fa-tags"></i> {{ __('messages.categorykuliner') }}</strong>
                  
                                    <span
                                                    class="badge badge-pill badge-kuning">{{ $kuliner->getCategory->category_name }}</span>
                  
                                  <hr>
                  
                                  <strong><i class="fas fa-map-marker-alt mr-1"></i> {{ __('messages.address') }}</strong>
                  
                                  <p class="text-muted">{{ $kuliner->alamat }}</p>
                                  ...
                                  <strong><i class="fas fa-clock mr-1"></i> {{ __('messages.operational') }}  <?php
                                    ...
                                    $jamBuka = \Carbon\Carbon::createFromTimeString($kuliner->jambuka);
                                    $jamTutup = \Carbon\Carbon::createFromTimeString($kuliner->jamtutup);
                                    // dd($jamTutup);
                                    ?>
                                ...
                                {{ $kuliner->jambuka }} S.d {{ $kuliner->jamtutup }}
                                  ...
                                  <span class="badge badge-pill badge-biru">{{ $kuliner->kapasitas }}</span>
                                  <hr>

                                  <strong><i class="fas fa-bullhorn"></i> Sosial Media</strong>
                                  <p class="text-muted"> <a href="{{ $kuliner->instagram }}" class="instagram"> <button type="submit"
                                    class="btn btn-outline-orange"><i
                                        class="bx bxl-instagram icon-large"></i></button></a>
                                    <a href="{{ $kuliner->web }}"> <button type="submit"
                                            class="btn btn-outline-orange"><i
                                                class="bx bxl-opera icon-large"></i></button></a></p>
                                                <hr>
                                                <strong><i class="fas fa-receipt"></i> {{ __('messages.MenuList') }}</strong>
                                                <p>
                                                    @foreach ($kulinerproduk as $ticketCategory)
                                                        <span class="badge badge-pill badge-orange">
                                                    ...
                                                    @if (!$kulinerproduk->isEmpty())
                                                    <div data-v-a81f42ae="" class="row mt-3">
                                                        <form action="{{ route('website.pesankuliner', ['kuliner_id' =>  $hash->encodeHex($kuliner->id)]) }}" method="GET">
                                                            <button type="submit" class="btn btn-outline-orange">{{ __('messages.Reservekuliner') }}</button>
                                                        </form>
                                                    </div>
                                                @endif
                                                </p>
                                        ...
                                        <hr>
                                        <a href="https://wa.me/{{ preg_replace('/^0/', '62', $kuliner->telpon) }}"
                              ...
                              src="https://maps.google.com/maps?q={{ $kuliner->latitude }},{{ $kuliner->longitude }}&output=embed"></iframe>
                              <a 
                                  href="https://www.google.com/maps/dir/?api=1&destination={{ $kuliner->latitude }},{{ $kuliner->longitude }}"
                                  class="btn btn-outline-map text-white text-center w-100 fs-15px mt-3"
                                  style="border-radius: 15px; padding: 15px;"onclick="arahkanSaya('{{ $kuliner->latitude }}', '{{ $kuliner->longitude }}')">{{ __('messages.directMe') }}</button></a>
    ...
    .star5:before {
        ...
        color: #ffc700; /* Warna bintang untuk rating 5 */
                    ...
                    <div class="row mt-3">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{ route('website.review.updatekuliner', ['kuliner' => $hash->encodeHex($kuliner->id), 'id' => $userReview->id]) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                        ...
                        <form action="{{ route('website.review.storekuliner', ['kuliner' => $hash->encodeHex($kuliner->id)]) }}" method="POST">
                            @csrf
                            <div class="form-group">
                                <label for="comment">{{ __('messages.comment') }}</label>
