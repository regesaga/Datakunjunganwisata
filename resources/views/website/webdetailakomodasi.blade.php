        ...
        <section id="breadcrumbs" class="breadcrumbs">
            <div class="container">
                ...
                <ol>
                    ...
                    <li>Akomodasi Details</li>
                </ol>

            </div>
        </section><!-- End Breadcrumbs -->

        <!-- ======= Akomodasi Details Section ======= -->
                        ...
                        class="p-3 rounded-patern mt-3">
                          ...
                          <div class="container">
                            <div class="row" style="margin-top:50px; margin-bottom:40px; margi-left:0px;">
                                        ...
                                        <div class="portfolio-details-slider swiper">
                                            <div class="swiper-wrapper align-items-center">
                                                ...
                                                @foreach ($item->photos as $key => $media)
                                                    <div class="swiper-slide">
                                                        <a href="{{ $media->getUrl() }}" target="_blank">
                                                        <img src="{{ $media->getUrl('thumb') }}"  style="border-radius: 10px;" class="uwaw img-rooms-mobile" width="350px" height="200px">
                                                        </a>
                                                    </div>
                                                @endforeach
                                            </div>
                                    ...
                                    <div class="col-md-6 box-desc-rsvp">
                                        <div class="row">
                                            <div class="col-md-12 col-xs-12 boxrs-disable ml-20 mt-15"
                                                style="box-shadow: 6px 6px 6px 6px  #888888; border-radius: 10px;">
                                                <div class="col-md-12">
                                                    <div class="row">
                                                        <h2 style="align-items: center"><strong> {{ $item->getCategory->category_name ?? '' }}</strong></h2>
                                                        <hr>
                                                        
                                                            <p><span class="badge badge-pill badge-biru">{{$item->kapasitas }}  {{ __('messages.Person') }}</span>
                                                            King / Queen / Double </p>
                                                            <p>From <span class="badge badge-pill badge-orange"> <strong>Rp. {{ number_format($item->harga, 0, ".", ".") }},-</strong>/{{ __('messages.Night') }} </span></p>
                                                        <hr>
                                                        
                                                        <div class="col-md-12 col-xs-12" style="margin-bottom: 20px">
                                                            <strong><i class="fas fa-person-booth"></i> {{ __('messages.Facilities') }}</strong>
                                                            <p>
                                                            @foreach ($item->fasilitas as $key => $fasilitas)
                                                            <span class="badge badge-pill badge-toska">{{ $fasilitas->fasilitas_name }}</span>
                                                            @endforeach
                                                            </p>
                                                        </div>
                                                    </div>
        ...
        <!-- End Room  -->
                    ...
                    <div class="col-lg-5">
                        <span class="fas fa-eye mr-1"></span>
                            {{ __('messages.webView') }} {{ $akomodasi->views }}
                            
                            
                            <div class="card card-primary">
                                <div class="card-header">
                                  <h3 class="card-title">Akomodasi information</h3>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <strong><i class="fas fa-tags"></i> {{ __('messages.categoryacomodation') }}</strong>
                  
                                    <span class="badge badge-pill badge-kuning">{{ $akomodasi->getCategoryAkomodasi->category_name }}</span>
                    
                                    <hr>
                                    
                                  <strong><i class="fas fa-map-marker-alt mr-1"></i> {{ __('messages.address') }}</strong>
                  
                                  <p class="text-muted">{{ $akomodasi->alamat }}</p>
                                  ...
                                  <strong><i class="fas fa-clock mr-1"></i> {{ __('messages.operational') }}  <?php
                                    ...
                                    $jamBuka = \Carbon\Carbon::createFromTimeString($akomodasi->jambuka);
                                    $jamTutup = \Carbon\Carbon::createFromTimeString($akomodasi->jamtutup);
                                    // dd($jamTutup);
                                    ?>
                                ...
                                {{ $akomodasi->jambuka }} S.d {{ $akomodasi->jamtutup }}
                                  ...
                                  <span class="badge badge-pill badge-biru">{{ $akomodasi->kapasitas }}</span>
                                  <hr>
                                  <strong><i class="fas fa-person-booth"></i> {{ __('messages.Facilities') }}</strong>
                                  <p>
                              ...
                              <p class="text-muted"> <a href="{{ $akomodasi->instagram }}" class="instagram"> <button type="submit"
                                class="btn btn-outline-orange"><i
                                    class="bx bxl-instagram icon-large"></i></button></a>
                                <a href="{{ $akomodasi->web }}"> <button type="submit"
                                        class="btn btn-outline-orange"><i
                                            class="bx bxl-opera icon-large"></i></button></a></p>
                                    <hr>
                                                <strong><i class="fas fa-receipt"></i> {{ __('messages.RoomList') }}</strong>
                                                <p>
                                                    @foreach ($room as $ticketCategory)
                                                        <span class="badge badge-pill badge-orange">
                                                            @if ($ticketCategory->harga == 0)
                                                                {{ __('messages.Free') }}
                                                            @else
                                                                {{ $ticketCategory->nama }} Rp. {{ number_format($ticketCategory->harga, 0, ".", ".") }},-
                                                            @endif
                                                        </span>
                                                        <br>
                                                    @endforeach
                                                    ...
                                                    <div data-v-a81f42ae="" class="row mt-3">
                                                        <form action="{{ route('website.reserv', ['akomodasi_id' =>  $hash->encodeHex($akomodasi->id)]) }}" method="GET">
                                                            <button type="submit" class="btn btn-outline-orange">{{ __('messages.Reservation') }}</button>
                                                        </form>
                                                    </div>
                                                @endif
                                                </p>
                                        ...
                                        <hr>
                                        <a href="https://wa.me/{{ preg_replace('/^0/', '62', $akomodasi->telpon) }}"
                              ...
                              src="https://maps.google.com/maps?q={{ $akomodasi->latitude }},{{ $akomodasi->longitude }}&output=embed"></iframe>
                              <a 
                                  href="https://www.google.com/maps/dir/?api=1&destination={{ $akomodasi->latitude }},{{ $akomodasi->longitude }}"
                                  class="btn btn-outline-map text-white text-center w-100 fs-15px mt-3"
                                  style="border-radius: 15px; padding: 15px;"onclick="arahkanSaya('{{ $akomodasi->latitude }}', '{{ $akomodasi->longitude }}')">{{ __('messages.directMe') }}</button></a>
    ...
    .star5:before {
        ...
        color: #ffc700; /* Warna bintang untuk rating 5 */
                    ...
                    <div class="row mt-3">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{ route('website.review.updateakomodasi', ['akomodasi' => $hash->encodeHex($akomodasi->id), 'id' => $userReview->id]) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                        ...
                        <form action="{{ route('website.review.storeakomodasi', ['akomodasi' => $hash->encodeHex($akomodasi->id)]) }}" method="POST">
                            @csrf
                            <div class="form-group">
                                <label for="comment">{{ __('messages.comment') }}</label>
