                ...
                <thead>
                    <tr><th colspan="{{ 3 + (count($kelompok) * 2) + (count($wismannegara) * 2) }}">
                        <h2 style="text-align: center; text-transform: uppercase;">
                            Rekap Data Kunjungan {{$akomodasi->namaakomodasi}} perbulan Tahun {{ $tahun }}
                        </h2>
                        </th>
                    </tr>
                    <tr>
...
<script>
    document.getElementById('download-to-pdf').addEventListener('click', function () {
        ...
        var opt = {
            margin:       [10, 10, 10, 10],  // Menambahkan margin atas, kanan, bawah, kiri (dalam mm)
            filename:     'Kunjungan_Akomodasi_dalamsatutahun' + new Date().toISOString() + '.pdf',
            image:        { type: 'jpeg', quality: 0.98 },
            html2canvas:  { scale: 3 },  // Meningkatkan kualitas gambar
            jsPDF:        { 
                unit: 'mm', 
...
<script>
    document.getElementById('download-to-excel').addEventListener('click', function () {
        ...
        var sheet = XLSX.utils.table_to_book(table, { sheet: 'Kunjungan Akomodasi Dalam Satu Tahun' }); // Konversi tabel menjadi buku Excel
        var filename = 'Kunjungan_Akomodasi_dalamsatutahun_' + new Date().toISOString() + '.xlsx'; // Nama file dengan timestamp
