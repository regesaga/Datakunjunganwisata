...
<section class="content-header">
    <div class="container-fluid">
        <form action="{{ route('account.akomodasi.kunjunganevent.update', $tanggal_kunjungan) }}" method="POST">
            @csrf
            @method('PUT')  <!-- Ganti dari PATCH ke PUT -->
            <div class="form-header">
                <h1  style="text-align: center; text-transform: uppercase;">Edit Data Kunjungan Event</h1>
                <h2  style="text-align: center; text-transform: uppercase;">{{ $event->title }}</h2>
                @if (session('warning'))
            ...
            <div class="col-lg-12">
                <input type="hidden" name="event_calendar_id" value="{{ $hash->encode($event->id) }}" required>
