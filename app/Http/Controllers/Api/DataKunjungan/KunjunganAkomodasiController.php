    ...
    public function createWisnu()
{
    ...
    // Ambil hanya kolom yang diperlukan dari model Akomodasi
       // Ambil hanya kolom yang diperlukan dari model Akomodasi
...
public function editWisnuApi($akomodasi_id, $tanggal_kunjungan)
{
    // Langsung menggunakan akomodasi_id tanpa decode
    if (!$akomodasi_id) {
        return response()->json(['error' => 'Akomodasi ID tidak valid.'], 404);
    }
    ...
    // Ambil data Wisnu dan Wisman berdasarkan akomodasi_id dan tanggal_kunjungan
    ...
    // Ambil data kelompok dan negara akomodasi
    return response()->json([
        'data' => [
            'akomodasi_id' => $akomodasi->id,
            'akomodasi' => $akomodasi->namaakomodasi,
...
public function updateWisnuApi(Request $request, $tanggal_kunjungan)
{
    ...
    // Mulai transaksi
    DB::beginTransaction();
    Log::info('Starting updateWisnuApi method', ['tanggal_kunjungan' => $tanggal_kunjungan, 'akomodasi_id' => $akomodasi_id]);

    try {
        // Hapus data sebelumnya berdasarkan akomodasi_id dan tanggal_kunjungan
        ...
        if ($request->filled('wismannegara_id') && $request->filled('jml_wisman_laki') && $request->filled('jml_wisman_perempuan')) {
            foreach ($request->wismannegara_id as $index => $negara) {
                ...
                if (!$wismanExists) {
                    return response()->json(['error' => 'Akomodasiwan Mancanegara dengan ID ' . $negara . ' tidak ditemukan.'], 400);
                }
