...
use App\Http\Resources\AkomodasiResource;
...
class AkomodasiApiController extends Controller
{
    ...
    {
        //get all posts
        $dataakomodasi = Akomodasi::where('active', 1)->with(['room', 'fasilitas', 'created_by'])->get();
    // Loop melalui setiap Akomodasi untuk mendapatkan review dan rata-rata rating
    $akomodasis = $dataakomodasi->map(function($akomodasi) {
        ...
        // Mengkonversi model Akomodasi ke AkomodasiResource
        $akomodasiResource = new AkomodasiResource($akomodasi);
        $akomodasiResource->average_rating = $averageRating;

        return $akomodasiResource;
    });
    return response()->json([
        'message' => 'Data Akomodasi',
        'success' => true,
        'data' => $akomodasis
    ], 200);
    ...
    {
        if (isset($id)) {
            $akomodasi = Akomodasi::with(['room' => function($query) {
            ...
            return response()->json([
                'message' => 'Akomodasi data retrieved successfully',
                'success' => true,
                'data' => [
                    'akomodasi' => $akomodasi,
            ...
            $akomodasis = Akomodasi::with(['room' => function($query) {
                $query->where('active', 1);
            }, 'fasilitas'])->get();
            return response()->json(['messages' => 'Data Akomodasi', 'success' => true, 'data' => $akomodasis], 200);
        }
    }
    ...
    public function updateReview(Request $request, $id, $reviewId)
{
    ...
    try {
        $review = ReviewRatingAkomodasi::findOrFail($reviewId);
